...
def _add_generic(building, name: str, value: str, gen_ns: str):
    ...
    node.set("name", name)
    v = etree.SubElement(node, f"{{{GEN2_NS}}}value")
    v.text = value
                    ...
                    if b is not None:
                        ...
                        if gid in selected and gid not in written:
                            ...
                            if embed_generic:
                                ...
                                meta = selected[gid]
                                _add_generic(bb, "heritageComplexId", ";".join(meta["complex_ids"]), gen_ns)
                                _add_generic(bb, "heritageComplexName", ";".join(meta["complex_names"]), gen_ns)
                                _add_generic(bb, "heritageMatchMethod", ";".join(meta["methods"]), gen_ns)
                            xf.write(member)
                            written.add(gid)
                    elem.clear()
                    parent = elem.getparent()
