...
Detail maps use OpenStreetMap tiles through contextily at zoom 16.
"""
...
def overlay_heritage(ax, footprints, points):
    if not footprints.empty:
        footprints.boundary.plot(ax=ax, linewidth=1.0, color="black", alpha=0.9, zorder=5)
    if not points.empty:
        points.plot(ax=ax, markersize=15, color="black", marker="o", zorder=6)
...
def detail_inundation(source: Path, contents: pd.DataFrame, center_name: str, lat: float, lon: float, radius: float, outdir: Path, locations):
    ...
    footprints, points = read_detail_context(source, locations, b4326)
    layers = []
    for _, r in contents[contents["table_name"].str.startswith("hazard_inundation_", na=False)].iterrows():
    ...
    for layer in layers:
        ...
        hz = hz.to_crs(3857)
        hz.plot(ax=ax, column="inundation_depth_m", cmap=cmap, norm=norm, markersize=4, alpha=0.65, zorder=2)
    ...
    add_discrete_legend(ax, cmap, DEPTH_LABELS, "想定浸水深")
    ax.text(0.01, 0.01, "重ね合わせ: " + ", ".join(used), transform=ax.transAxes, fontsize=6, va="bottom", zorder=10)
    fig.tight_layout(); fig.savefig(outdir / f"{center_name}_inundation.png", dpi=220, bbox_inches="tight"); plt.close(fig)
