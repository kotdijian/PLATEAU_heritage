...
1. 直接CSV URLを試行
2. CSVが404・接続失敗等で取得できない場合、API endpointがあれば自動fallback
3. CSV URLがない場合は東京都Open Data API JSON endpoint
4. 取得可能ソースがなければ記録して継続

東京都Open Data APIへのPOSTは `Accept: application/json` / `Content-Type: application/json` とJSON body `{}` を使用します。`limit` / `offset` で `total` までページングし、取得結果を単一の `data.json` の `hits` にまとめます。既定page sizeは1000で、必要なら `--api-page-size` で変更できます。

既存 `data.csv` / `data.json` は通常再取得しません。したがって一度成功した自治体を保持したまま、失敗自治体だけ再試行できます。全件再取得時だけ `--overwrite` を指定します。

杉並区 `13115` のmanifestは2026年4月更新版CSVへ更新済みです。
...
v0.5.1では `movable` を特別な空間処理ルートとして扱いません。
...
v0.5.1では `heritage_movable_items` / `heritage_movable_groups` は生成しません。
...
- v0.1.0で既に生成済みの `national.csv` / `municipal.csv` には `address_detail` がないため、必要ならv0.2.2で再正規化する。
