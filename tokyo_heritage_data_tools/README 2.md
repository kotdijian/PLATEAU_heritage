...
> **v0.2.1**: `heritage-collect municipal --area-code 13` を拒否していたコード検証の正規表現バグを修正しました。v0.2.0をインストール済みの場合はv0.2.1へ更新してください。
...
既存 `data.csv` / `data.json` はデフォルトで再取得しません。再取得時だけ `--overwrite` を使います。
