...
`build_museum_hazard_gpkg.py` v0.5.0は、MuseumソースmanifestとPLATEAU CityGMLの建築物を照合し、既存のHeritage災害リスクGeoPackageを複製した上で、博物館向けの空間レイヤ・正規化テーブル・災害リスクを追加します。`museum_location_enrichment.csv`の受理済み住所・座標を照合用overlayとして優先し、原manifest住所は`source_manifest_address`に保存します。博物館の所在階・収蔵庫階は建物階数と分離して保持し、浸水深との対応表を生成します。

v0.4.0ではOSMパイロットv0.1.2の高確度純増候補をPLATEAU footprint照合へ接続しました。v0.5.0では未確定対象を、複数棟、footprint非該当、座標競合、OSM候補のみの4群へ整理し、近傍PLATEAU footprintを`museum_manual_review_buildings`へ出力します。`render_museum_review.py`で選択画面を生成し、判断CSVを次回buildへ再投入できます。
...
| レイヤ／テーブル | 内容 |
|---|---|
| `museum_manual_review_buildings` | 施設ごとに絞り込んだPLATEAU footprint（既定上位12棟） |
| `museum_manual_review_queue` | 複数棟、位置ずれ、座標競合、OSM候補のみのレビュー順 |
| `museum_building_overrides_template` | 手動確定CSVと同じ列を持つ空テンプレート |

レビューHTMLは次のコマンドで生成します。renderer v0.3.0は外部JavaScriptやCDNに
依存せず、GPKG内のfootprintをSVGとして表示します。オンライン時は地理院標準地図
タイルを背景に読み込み、チェックボックスで背景を切り替えられます。タイルを取得
できない場合も候補ポリゴンと選択機能は維持されます。

```bash
python Museum/render_museum_review.py \
  "/path/to/13_museum_hazards_review.gpkg"
```

ブラウザ画面で施設と建物を選択し、「判断CSVをダウンロード」すると
`museum_building_overrides.csv`が得られます。確定結果を反映するには次回buildで指定します。
...
python Museum/build_museum_hazard_gpkg.py \
  ...
  --plateau-local-dir .cache/plateau \
  --exclude-unresolved-duplicates \
  --manual-overrides "/path/to/museum_building_overrides.csv" \
  --output "/path/to/13_museum_hazards_reviewed.gpkg"
```

`confirm`だけが反映され、未知のmuseum ID・Building IDはエラーになります。複数棟施設は
同じmuseum IDについて複数行を`confirm`できます。
