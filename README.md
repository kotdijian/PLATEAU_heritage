...
- `Tokyo Heritage Data Tools` v0.2.0
...
Data Tools v0.2.0では `方書` 等を `address_detail` として保持します。これはComplex判定の補助情報になりますが、Data Tools側ではBuildingや範囲を推定しません。

### Tokyo Heritage Data Tools v0.2.0との整合

Data Tools v0.2.0 は Extractor v0.5.x に合わせて更新されています。
...
Extractor v0.5.0 と Tokyo Heritage Data Tools v0.2.0 は、次の所在地詳細列を `address_detail` として扱えるよう整合しています。
...
- Data Tools v0.2.0で正規化する国・自治体データも、元データに対応列があれば `address_detail` として保持する。
- `address_detail` はComplex判定の補助情報であり、Data Tools側では範囲やBuildingを推定しない。
- v0.1.0で既に生成済みの `national.csv` / `municipal.csv` には `address_detail` がないため、必要ならv0.2.0で再正規化する。

---

## 23. `entity_class` の扱い

Tokyo Heritage Data Tools v0.2.0 はcanonical CSVに:
...
- `Tokyo Heritage Data Tools v0.2.0` は東京都向けプロトタイプであり、東京都内自治体ソースmanifest・東京都自治体コード判定を含む。
