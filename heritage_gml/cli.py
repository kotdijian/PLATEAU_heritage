...
def main(argv: list[str] | None = None) -> None:
    # Parse lightweight options first. argparse handles --version by exiting here,
    # so pandas/pyproj/geopandas and the pipeline are not imported for version checks.
    args = _parser().parse_args(argv)

    # Heavy imports are intentionally delayed until an actual extraction run.
